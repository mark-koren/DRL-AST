...
class GaussianDeepLSTMPolicy(StochasticPolicy, LayersPowered, Serializable):
    def __init__(
            self,
            name,
            env_spec,
        ...
        :param hidden_lstm_dim: dimension of hidden layer
        :param hidden_nonlinearity: nonlinearity used for each hidden layer
        :return:
        """
        with tf.variable_scope(name):
            Serializable.quick_init(self, locals())
            super(GaussianDeepLSTMPolicy, self).__init__(env_spec)
            ...
            output_dim = output_network.output_layer.output_shape[-1]
            self.l_flat_output = output_network.output_layer
            ...
            output_input_var = tf.placeholder(dtype=tf.float32, shape=(None, hidden_dim), name="output_input")
            output_var = L.get_output(self.l_flat_output, {output_network.input_layer: output_input_var})
            obs_var = output_network.input_layer.input_var
            self.f_get_output = tensor_utils.compile_function(
                inputs=[output_input_var],
                outputs=output_var,
            )
            ...
            out_layers = [output_network.output_layer, mean_network.output_layer, l_log_std]
    ...
    def dist_info_sym(self, obs_var, state_info_vars):
        ...
        if self.feature_network is None:
            lstm_out, log_stds = L.get_output(
        ...
        pdb.set_trace()
        means = L.get_output(self.l_flat_output, {self.output_network.input_layer: lstm_out})
    ...
    def reset(self, dones=None):
        ...
        if self.prev_actions is None or len(dones) != len(self.prev_actions):
            ...
            self.prev_hiddens = np.zeros((len(dones), self.hidden_lstm_dim))
            self.prev_cells = np.zeros((len(dones), self.hidden_lstm_dim))
    ...
    def get_actions(self, observations):
        ...
        lstm_out, log_stds, hidden_vec, cell_vec = self.f_step_mean_std(
            all_input, np.hstack([self.prev_hiddens, self.prev_cells]))
        means = self.f_get_output(lstm_out)
